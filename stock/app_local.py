...
MY_PORTFOLIO = [
    ...
    {"名稱": "富邦特選高股息30", "平均成本": 12.31, "股數": 1000},
]

MOM_PORTFOLIO = [
    {"名稱": "元大高股息", "平均成本": 29.89, "股數": 15000},
    {"名稱": "中鋼", "平均成本": 21.07, "股數": 10604},
    {"名稱": "聯電", "平均成本": 0.0, "股數": 654},
    {"名稱": "台積電", "平均成本": 580.0, "股數": 100},
    {"名稱": "宏碁", "平均成本": 25.09, "股數": 338},
    {"名稱": "中信金", "平均成本": 21.53, "股數": 1000},
    {"名稱": "群創", "平均成本": 0.0, "股數": 531},
    {"名稱": "方土霖", "平均成本": 7.15, "股數": 19374},
    {"名稱": "彩晶", "平均成本": 0.0, "股數": 851},
]
...
def process_portfolio(portfolio, year):
    processed = []
    for item in portfolio:
        ...
        if stock_code:
            ...
            dividend = get_dividend_info(stock_code, year=year)
            cash_dividend = dividend["現金股利"]
            stock_dividend = dividend["股票股利"]
            total_cash = round(cash_dividend * qty, 2)
            total_stock = round((stock_dividend / 10) * qty, 2)
            stock_value = round(total_stock * now_price, 2) if now_price else 0
...
def index():
    ...
    portfolio_data = process_portfolio(MY_PORTFOLIO, selected_year)
    mom_data = process_portfolio(MOM_PORTFOLIO, selected_year)

    def get_summary(data):
        return {
            "總成本": sum(float(p["總成本"]) for p in data),
            "現值": sum(float(p["現值"]) for p in data),
            "損益": round(sum(float(p["損益"]) for p in data), 2),
            "總現金股利": round(sum(float(p["總現金股利"]) for p in data), 2),
            "總股票股利": round(sum(float(p["總股票股利"]) for p in data), 2),
            "總股票股利市值": round(sum(float(p["股票股利市值"]) for p in data), 2)
        }

    summary = get_summary(portfolio_data)
    mom_summary = get_summary(mom_data)

    taipei_time = datetime.now(pytz.timezone("Asia/Taipei")).strftime("%Y-%m-%d %H:%M:%S")
    combined_summary = get_summary(portfolio_data + mom_data)
    return render_template(
        "index.html",
        portfolio=portfolio_data,
        mom_portfolio=mom_data,
        updated_time=taipei_time,
        summary=summary,
        mom_summary=mom_summary,
        combined_summary=combined_summary,  # ⬅️ 傳入模板
